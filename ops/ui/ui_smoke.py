        ...
        for name in PAGES:
            ...
            try:
                ...
                for kw in ["Traceback", "TypeError", "ValueError", "Exception:"]:
                    if kw in txt:
                        errors.append(kw)
                for kw in ["Aucun","Aucune","not found","No data","pas de données"]:
                    if kw in txt:
                        warns.append(kw)
...
def main() -> int:
    try:
        ...
        print(json.dumps({"ok": False, "error": "playwright not installed","hint": "pip install playwright && python -m playwright install chromium"}))
        return 1
